...
import axios, { GET_ALL_BROKERS } from '@/api/axios';
  ...
  return (
    ...
    <div className="mb-8 flex flex-col gap-12">
      <div className="container max-auto px-4 py-8">
        <Link to={'/dashboard/brokers/add-new'} className="block md:inline-block mb-4">
        ...
        <div className="-mx-4 overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Broker Name / Image</th>
                ...
                <tr key={broker._id}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <img  className="rounded-full w-20 h-20" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQV3XeKXPIxJP-R6Hg0d2x2DCxnKV_sT04umGCOTuiNIQ&s" alt="image description" />
                      <div className="ml-4">{broker.fullName}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">{broker.email}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{broker.contactNumber}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{broker.address}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{broker.city}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Button onClick={() => handleDelete(broker._id)} className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded md:mr-2" color='red'>
                      Delete
                    </Button>
